...
<native:scroll-view class="w-full h-full bg-gray-50">
    <native:column class="w-full gap-4 p-6">
        ...
        @else
            <native:column class="w-full p-3 bg-blue-50 rounded-xl">
            ...
            @forelse ($offers as $offer)
                <native:column key="offer-{{ $offer['id'] }}" class="w-full p-4 bg-white rounded-xl gap-2">
                    ...
                    <native:row class="gap-4">
                        <native:text class="text-lg font-bold text-blue-600">${{ $offer['fee'] }}</native:text>
            ...
            @forelse ($nearbyProviders as $provider)
                <native:row key="nearby-{{ $provider['id'] }}" class="w-full p-3 bg-white rounded-xl justify-between">
